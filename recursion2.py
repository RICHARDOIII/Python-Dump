...
#  Description:Second Recursion tests

#  Student Name:Daniel Espinoza	

#  Student UT EID:dee467

#  Partner's Name:Mark Simmons

#  Partner's UT EID:mjs5839
...
# base case; if Target is zero then the case is final
	if target==0:
		return True;
# the start cannot be larger		
	elif (start>= len(nums)):
		return False
# so it goes throught this  and skips over the adjacent value
	elif(groupNoAdj(start+2,nums,target-nums[start])):
		return True	
	else:
# this goes to the next intial element
...
def groupSumClump(start, nums, target):
 ...
 # so we start with the adjacent value
  		clump=start+1
 # if the clump is good we should use it if not completely skip all 
 # them by going the distance of the clump + start
  		for x  in range(clump,len(nums)):
  			if nums[start] ==nums[x]:
  				clump+=1
  		if (groupSumClump(clump, nums, target - ((clump - start) * nums[start]))):
  			return True 
  # Theres no point of staying in the clump area if it doesn't work
  		return groupSumClump(clump,nums,target)
...
def splitArrayHelper(nums, start, a, b):
  #print(a, b) #-debugging
  if start == len(nums):
    return sum(a) == sum(b)
  else:
    a2 = a[:]
    a2.append(nums[start])
    b2 = b[:]
    b2.append(nums[start])
    return splitArrayHelper(nums, start+1, a2, b) or \
    splitArrayHelper(nums, start+1, a, b2)
...
def splitOdd10Helper(nums,start,a,b):
	if start == len(nums):
# if we get to the end the we append 
# last element to create the b array
		b_sum=sum(b)
		b.append(b_sum)
	else:
# will equal all the elements in a
		c = a[:]
#append the element
		a.append(nums[start])
		splitOdd10Helper(nums,start+1,a,b)
		splitOdd10Helper(nums,start+1,c,b)
#make sure the parted groups meet the conditions	
	return (sum(a)%10==0 and sum(b)%2==1) or \
	(sum(b)%10==0 and sum(a)%2==1)
...
def split53Helper(nums, start, a, b):
  if start == len(nums):
    return sum(a) == sum(b)
  elif nums[start] % 5 == 0:
    a.append(nums[start])
    return split53Helper(nums, start+1, a, b)
  elif nums[start] % 3 == 0:
    b.append(nums[start])
    return split53Helper(nums, start+1, a, b)
  else:
    ...
    b2 = b[:]
    b2.append(nums[start])
    return split53Helper(nums, start+1, a2, b) or \
    split53Helper(nums, start+1, a, b2)
